...
public class LakesidePrintJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var streamSource1 = KafkaReader.GetDataStreamSource(env, TOPIC_1, GROUP_ID);
        streamSource1.map(event -> event.Type + ": " + event.Content.toString()).print();

        var streamSource2 = KafkaReader.GetDataStreamSource(env, TOPIC_2, GROUP_ID);
        streamSource2.map(event -> event.Type + ": " + event.Content.toString()).print();

        var streamSource3 = KafkaReader.GetDataStreamSource(env, TOPIC_3, GROUP_ID);
        streamSource3.map(event -> event.Type + ": " + event.Content.toString()).print();

        var streamSource4 = KafkaReader.GetDataStreamSource(env, TOPIC_4, GROUP_ID);
        streamSource4.map(event -> event.Type + ": " + event.Content.toString()).print();

        var streamSource5 = KafkaReader.GetDataStreamSource(env, TOPIC_5, GROUP_ID);
