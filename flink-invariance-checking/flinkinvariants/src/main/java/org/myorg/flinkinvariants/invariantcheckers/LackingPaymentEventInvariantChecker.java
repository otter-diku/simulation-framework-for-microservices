...
public class LackingPaymentEventInvariantChecker {
    ...
    final static OutputTag<InvariantViolationEvent> outputTag = new OutputTag<>("lacking-payments") {};
    ...
    public static void CheckLackingPaymentInvariant(
        ...
        var distinctMatches = matches.getSideOutput(outputTag)
                .filter(new FilterFunction<InvariantViolationEvent>() {
    ...
    public static class MyPatternProcessFunction extends PatternProcessFunction<EShopIntegrationEvent, InvariantViolationEvent> implements TimedOutPartialMatchHandler<EShopIntegrationEvent> {
        @Override
        public void processMatch(Map<String, List<EShopIntegrationEvent>> map, Context context, Collector<InvariantViolationEvent> collector) throws Exception {
            // TODO: can we just do nothing here? we are not really interested when the pattern completes before the timeout, right?
        }
